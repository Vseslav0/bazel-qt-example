...
void ProgressBar::ValueChange() {
    if (parent->listwidget->current_ticket->question_view->yellow) {
        if ((parent->listwidget->current_ticket->question_view->status->currentText() != "lime"
             && (parent->listwidget->current_ticket->question_view->status->currentText() != "yellow"))) {
            this->setValue(this->value() - 1);
...
QuestionView::QuestionView(QString title, QString number_) : name(new QLabel(title)), number(new QLabel(number_))
    , name_edit(new QLineEdit(title)){
...
void ListWidget::AddElement(int val) {
    ...
    while (i <= val) {
        ...
        this->addItem(str);
        connect(current_ticket->question_view->status, &QComboBox::currentTextChanged, this, &ListWidget::StatusChange);
        connect(current_ticket->question_view->status, &QComboBox::currentTextChanged, parent->green_progress_bar, &ProgressBarGreen::ValueChange);
        connect(current_ticket->question_view->status, &QComboBox::currentTextChanged, parent->progress_bar, &ProgressBar::ValueChange);
        connect (current_ticket->question_view->name_edit, &QLineEdit::editingFinished, this, &ListWidget::NameChanged);
...
void ListWidget::StatusChange() {
    this->currentItem()->setBackground(QBrush(QColor( this->current_ticket->question_view->status->currentText())));
}
